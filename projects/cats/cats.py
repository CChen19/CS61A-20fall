...
def about(topic):
    ...
    def select(paragraph):
        paralist = remove_punctuation(paragraph).lower().split()
        for t in topic:
            for p in paralist:
                if t == p:
                    return True
...
def pawssible_patches(start, goal, limit):
    ...
    # otherwise we don't know which edit operation is better
    else:
...
def fastest_words(game):
    ...
    for i in player_indices:
        fastest_lists.append([])
    
    for i in word_indices:
        fastest_player = 0
        for j in player_indices:
            if time(game, j, i) < time(game, fastest_player, i):
                fastest_player = j
        fastest_lists[fastest_player].append(word_at(game, i))
    return fastest_lists
    # END PROBLEM 10
