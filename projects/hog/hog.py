...
def swine_align(player_score, opponent_score):
    ...
    if num1 >= 10 and opponent_score != 0:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(*score):
        cur_increase = score[who] - last_score

        if cur_increase > running_high:
            print(cur_increase, "point(s)! The most yet for Player", who)
            return announce_highest(who, score[who], cur_increase)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        cur_average = average_roll_dice(i, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score+free_bacon_score, opponent_score) or free_bacon_score >= cutoff:
        return 0
    return num_rolls
    # END PROBLEM 11


def final_strategy(score, opponent_score, cutoff=12, num_rolls=5):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    free_bacon_score = free_bacon(opponent_score)
    score_after_free_bacon = score + free_bacon_score
    diff_from_goal = GOAL_SCORE - score
    diff_from_opp = score - opponent_score

    if extra_turn(score_after_free_bacon, opponent_score) or free_bacon_score >= cutoff or score_after_free_bacon >= GOAL_SCORE:
        return 0
    if diff_from_opp > 30:
        return 4
    if diff_from_opp > 15 and diff_from_goal <= 8:
        return 3
    return num_rolls
    # END PROBLEM 12
