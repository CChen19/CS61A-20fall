...
def store_digits(n):
    ...
    if n < 10:
        ...
        first_digit = n // 10**(num_digs - 1)
        rest_digits = n % 10**(num_digs - 1)
...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        return (t.label <= t.branches[0].label and is_bst(t.branches[0])) if t.label < bst_min(t.branches[0]) else (t.label >= t.branches[0].label and is_bst(t.branches[0]))
